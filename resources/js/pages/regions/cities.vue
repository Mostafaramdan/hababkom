            ...
            <table class="table table-striped table-dark table-bordered table-hover  mb-2"  >
                ...
                <tbody>
                    <tr v-for="(record,index) in records" :key="index">
                        ...
                        <td >
                            <button class="btn btn-info" @click="goDistricts(record.id)">
...
export default {
    ...
    methods:{
        ...
        create(){
             this.$router.push( {name:'regionsCreate', query: {'countries_id': this.$route.query.countries_id} });
        },
        goDistricts(id){
            this.$router.push({ name:'districts', query: { ...this.$route.query,'cities_id': id }});
        },
        update(index){
