<template>
    <div class="m-3" v-if="authorized.update">
        <div  @submit.prevent="onSubmit" class="border border-5 border-primary rounded form">
            <h3>
                {{$lang['update an apartments complex']}}
            </h3>
            <hr>
            ...
            <div class="form-check ">
                ...
                <treeselect v-model="record.attachs" :multiple="true" :options="attachments" ></treeselect>
            </div>
            <hr>
            <br>
            <div class="form-check ">
...
export default {
    ...
    methods: {
        ...
        async onSubmit() {
            ...
            let response = await this.Api('PUT',`apartments_complexes/${this.record.id}`,this.record);
    ...
    async mounted(){
        let res = await this.Api('GET',`apartments_complexes/${this.$route.params.id}`);
        this.record = res.data.record;
        this.record.payment = JSON.parse(this.record.payment)
        this.images = res.data.record.images;

        this.city= this.record.city.name_ar;
        this.record.attachs = this.record.attachs.map((el)=> el.id);
        let cities_id= this.record.city.regions_id;
    ...
    metaInfo() {
        return {
            title: `${this.$lang['app name']} - ${this.$lang['update an apartments complex']}`,
        }
    }
