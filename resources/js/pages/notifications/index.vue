        ...
        <div class="table-responsive">
            <table class="table table-striped table-dark table-bordered table-hover  mb-2"  >
                <thead >
                    <tr >
                        <th >#</th>
                        <th >المحتوي بالعربي </th>
                        <th >المحتوي بالانجليزي </th>
                        <th >تاريخ الانشاء  </th>
                        <th >#</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(record,index) in records" :key="index">
                        ...
                        <td>{{ new Date(record.created_at * 1000).getDate()}}/{{ new Date(record.created_at * 1000).getMonth() + 1}}/{{ new Date(record.created_at * 1000).getFullYear()}}</td>
                        <td>
...
export default {
    ...
    methods:{
        ...
        create(){
             this.$router.push( {name:'notificationsCreate',});
        },
        update(index){
             this.$router.push( {name:'notificationsUpdate', params: { id: this.records[index].id }});
        },
        show(index){
             this.$router.push( {name:'notificationsShow' , params: { id: this.records[index].id }});
        },
        ...
        async getRecords(){
            ...
            let response = await this.Api('GET','notifications',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`notifications/${this.records[index].id}`);
