...
export default {
    ...
    methods:{
        ...
        create(){
             this.$router.push( {name:'currenciesCreate',query:{...this.$route.query} });
        },
        update(index){
             this.$router.push( {name:'currenciesUpdate', params: { id: this.records[index].id }});
        },
        show(index){
             this.$router.push( {name:'currenciesShow' , params: { id: this.records[index].id }});
        },
        goToHousing(index){
        ...
        async getRecords(){
            ...
            let response = await this.Api('GET','currencies',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`currencies/${this.records[index].id}`);
