...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        async onSubmit() {
            this.loading=true;
            let response = await this.Api('PUT',`notifications/${this.record.id}`,this.record);
            this.loading=false;
    ...
    async mounted(){
        ...
        let response = await this.Api('GET',`notifications/${this.$route.params.id}`);
        ...
        metaInfo() {
        return {
            title: `${this.$lang['app name']} - ${this.$lang['update the notification']} `,
        }
    }
